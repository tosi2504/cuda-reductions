...
double timeKernel4(const int numThreadsPerBlock) {
    // set the datatype
...
int main () {
    ...
    for (int i = 0; i < 6; i++) {
        std::cout << numThreadsPerBlock << " " << timeKernel4(numThreadsPerBlock) << std::endl;
        file << numThreadsPerBlock << " " << timeKernel4(numThreadsPerBlock) << "\n";
